...
const LocationsCard = (props: CardProps<LocationRoot>): JSX.Element => {
  const { result } = props;

  return (
    <div className="flex flex-col justify-between border rounded-lg mb-4 px-6 py-4 shadow-lg bg-white">
      <div className="flex justify-between">
        <div className="w-3/4 font-semibold">{result?.rawData.name}</div>
        <div>
          {result?.distance && (result?.distance * 0.000621).toFixed(2) + " mi"}
        </div>
      </div>

      <div className="mt-4">{result?.rawData.address.line1}</div>
      <div>{result?.rawData.address.line2}</div>
      <div>{result?.rawData.address.city}</div>
      <div>{result?.rawData.address.region}</div>
      <div>{result?.rawData.address.postalCode}</div>
      <div className="mt-4">{result?.rawData.mainPhone}</div>
      <div className="flex mt-4 gap-x-5">
        <div className="px-4 py-1 bg-cyan-900 text-white">Get Directions</div>
        <div className="px-4 py-1 bg-red-700 text-white">Store Details</div>
      </div>
    </div>
  );
};
